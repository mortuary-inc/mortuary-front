...
const LogoText = () => {
  ...
  return (
    <div className={(isHomepage ? 'hidden md:block' : '') + ' text-left mr-4 lg:mr-8'}>
      <Link to={Routes.HomePage} className={(darkMode ? styles.darkMode : styles.logoModule) + ' basis-full"'}>
        <LogoTextSVG className=" cursor-pointer" />
      </Link>
      <div className={(darkMode ? 'text-third' : 'text-primary') + ' font-sansLight text-xs mt-3 basis-full'}>CURATING DEATH SINCE 2021</div>
    </div>
  );
};
