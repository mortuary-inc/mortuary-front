...
import NecrologyTableUser from './NecrologyTableUser';
// Import firestore helpers
import { doc, onSnapshot } from '@firebase/firestore';
// Import our db object
import { usersCollection } from 'apiFire';
...
const NecrologyRender = ({ withStats, owner, voxelID }: INecrologyRenderProps) => {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(doc(usersCollection, owner), (doc) => {
            let vi: ILogs = {
  ...
  return (
    <div id={NecrologyID} className="py-16 max-w-4xl mx-auto">
      <div className="text-left">
        ...
        <NecrologyTableUser itemPerTable={25} owner={owner} voxelID={voxelID} />
      </div>
    </div>
  );
};
